...
class AndorCameraController:
    ...
    def get_hsspeed(self):
        ...
        return self.cam.get_hsspeed()
    
    def get_hsspeed_frequency(self, hsspeed=None):
        return self.cam.get_hsspeed_frequency(hsspeed)
    ...
    def set_hsspeed(self, hsspeed):
        self.cam.set_amp_mode(hsspeed=hsspeed)
    ...
    def get_preamp(self):
        ...
        return self.cam.get_preamp()

    

    def get_preamp_gain(self, preamp=None):

        return self.cam.get_preamp_gain(preamp)
    ...
    def set_EMCCD_gain(self, gain, advanced=None):
        ...
        with self._lock:
            ...
            self.cam.set_EMCCD_gain(gain, advanced)
    ...
    def acquire_single(self):
        with self._lock:
            #actual_exposure = self.cam.get_exposure()   # read back from hardware, not the cache
            timeout = self.cam.get_exposure() + 2.0              # temporarily generous, to isolate the cause
            #print(f"Requesting snap: cached_exposure={self.exposure}, "
...
class KymeraController:
    ...
    def flipper_present(self, flipper):
    ...
    def get_grating_offset(self, grating=None):
        return self.spec.get_grating_offset(grating)

    def set_grating_offset(self, offset, grating=None):
        self.spec.set_grating_offset(offset, grating)

    def get_detector_offset(self):
        return self.spec.get_detector_offset()
    
    def set_detector_offset(self, offset):
        self.spec.set_detector_offset(offset)
...
class AndorSpectrometerDriver(BaseMeasurementInstrument):
    ...
    def settings(self):
        ...
        return {
            ...
            "vsspeed": self.camera.get_vsspeed(),

            "hsspeed frequency": self.camera.get_hsspeed_frequency(),

            "preamp gain": self.camera.get_preamp_gain(),
            ...
            "grating_offset": self.kymera.get_grating_offset(),
            "detector_offset": self.kymera.get_detector_offset(),
    ...
    def settings(self, settings):
        ...
        if "hsspeed" in settings:
            self.camera.set_hsspeed(settings["hsspeed"])

        if "grating_offset" in settings:
            self.kymera.set_grating_offset(settings["grating_offset"])
        
        if "detector_offset" in settings:
